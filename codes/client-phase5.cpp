...
void printMD5(unsigned char* md, long size = MD5_DIGEST_LENGTH) {
    for (int i=0; i<size; i++) {
        cout<< hex << setw(2) << setfill('0') << (int) md[i]<< dec;
    }
}
...
int requestconnectionthread(int no_neighbours,vector <int> neighbourid,vector <int> neighbourport,vector <string> search_filenames,string directory_path){
    ...
    // fileownerneighbourno[i] is the neighbour no corresponding to file i or port no of neightbour corresponding to file i;
        char buff[1024];
        memset(buff, '\0', 1024);
        strcat(buff,directory_path.c_str());
        strcat(buff,"Downloaded/");
        mkdir(buff, 0777);

    for(int i = 0; i<search_filenames.size(); i++){     
        if(fileownerdepth[i]==0)    continue;
        int corresponding_sockfd;
        if(fileownerdepth[i]==2){
            corresponding_sockfd = socket(PF_INET, SOCK_STREAM, 0);
            struct sockaddr_in their_addr;
            their_addr.sin_family = AF_INET;
            their_addr.sin_port=htons(fileownerneighbourno[i]);
            their_addr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
            memset(&(their_addr.sin_zero),'\0',8);
            while(true){
                if(connect(corresponding_sockfd, (struct sockaddr *) &their_addr, sizeof(struct sockaddr)) == 0) 
                    break;
                sleep(2);
            }
            memset(buf, '\0', 1024);
            int received=recv(corresponding_sockfd, buf, 1024, 0);
            }
        else corresponding_sockfd=mysideconnection_sockfd[fileownerneighbourno[i]];
        char buff[1024];
       
        memset(buff, '\0', 1024);
        strcat(buff,"SENDFILE");
        int sent=send(corresponding_sockfd,buff,1024,0);
        memset(buff, '\0', 1024);
        int received=recv(corresponding_sockfd,buff,1024,0);

        memset(buff, '\0', 1024);
        strcat(buff,search_filenames[i].c_str());
        sent=send(corresponding_sockfd,buff,1024,0);
        memset(buff, '\0', 1024);
        
        received=recv(corresponding_sockfd,buff,1024,0);
        int file_size=stoi(buff);
        memset(buff, '\0', 1024);
        strcat(buff,"FILESIZE RECEIVED");
        ...
        while(true){
            ...
            wrfile.write(buff,1024+file_size-received);
            break;
            }
            else
             wrfile.write(buff,1024);
        }

    if(fileownerdepth[i]==2){
        memset(buff, '\0', 1024);
        strcat(buff, "CLOSE");
        send(corresponding_sockfd, buff, 1024, 0);
        close(corresponding_sockfd);
    }

    }

    unsigned char file_hash[search_filenames.size()][MD5_DIGEST_LENGTH];
    for(int i = 0; i<search_filenames.size(); i++){     
         if(fileownerdepth[i]!=0){
            char filename[1024];
            memset(filename, '\0', 1024);
            strcat(filename, directory_path.c_str());
            strcat(filename,"Downloaded/");
            strcat(filename, search_filenames[i].c_str());
            FILE *file = fopen(filename, "rb");
            fseek(file, 0L, SEEK_END);
            int filesize = ftell(file);
            fseek(file, 0L, SEEK_SET);
            char* reader = new char[filesize];
            fread(reader, sizeof(char), filesize, file);
            MD5((unsigned char*)reader, filesize, file_hash[i]);
            delete reader;
            cout<<"Found "<<search_filenames[i]<<" at "<<fileowneruniqueid[i]<<" with MD5 ";
            for(int j = 0; j < MD5_DIGEST_LENGTH; j++){
                cout<< hex << setw(2) << setfill('0') << (int) file_hash[i][j] << dec;
            }
            cout<<" at depth "<<fileownerdepth[i]<<endl;
